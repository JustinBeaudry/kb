...
manifests with `kb sessions --unprocessed`, runs `kb summarize --json
<manifest>` lazily (requires the `claude` CLI or `KB_SUMMARIZE_COMMAND`),
retrieves each cached summary with `kb read-session summaries/<name>.md
--approve`, runs the ingest workflow on confirmed candidates, and finishes
each manifest with `kb mark-extracted`. It surfaces degraded summaries and
skipped summary-generation failures visibly, using `entire_checkpoint` for
